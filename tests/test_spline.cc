...
void testAll()
{
    ...
    /////////
    // full spline
    { Ewoms::Spline<double> sp(x[0], x[1], y[0], y[1], m0, m1); sp.set(x[0],x[1],y[0],y[1],m0, m1); testFull(sp, x, y, m0, m1); };
    { Ewoms::Spline<double> sp(2, x, y, m0, m1); sp.setXYArrays(2, x, y, m0, m1); testFull(sp, x, y, m0, m1);  };
    { Ewoms::Spline<double> sp(static_cast<size_t>(2), points, m0, m1); sp.setArrayOfPoints(2, points, m0, m1); testFull(sp, x, y, m0, m1); };
    /////////
    // test variable length splines
    /////////
    // full spline
    { Ewoms::Spline<double> sp(5, x, y, m0, m1); sp.setXYArrays(5,x,y,m0, m1); testFull(sp, x, y, m0, m1);  };
    { Ewoms::Spline<double> sp(xVec, yVec, m0, m1); sp.setXYContainers(xVec,yVec,m0, m1); testFull(sp, x, y, m0, m1);  };
    { Ewoms::Spline<double> sp; sp.setArrayOfPoints(5,points,m0, m1); testFull(sp, x, y, m0, m1); };
    { Ewoms::Spline<double> sp; sp.setContainerOfPoints(pointVec,m0, m1); testFull(sp, x, y, m0, m1);  };
    { Ewoms::Spline<double> sp; sp.setContainerOfTuples(pointsInitList,m0, m1); testFull(sp, x, y, m0, m1); };
    // natural spline
    { Ewoms::Spline<double> sp(5, x, y); sp.setXYArrays(5,x,y); testNatural(sp, x, y);  };
    { Ewoms::Spline<double> sp(xVec, yVec); sp.setXYContainers(xVec,yVec); testNatural(sp, x, y); };
    { Ewoms::Spline<double> sp; sp.setArrayOfPoints(5,points); testNatural(sp, x, y); };
    { Ewoms::Spline<double> sp; sp.setContainerOfPoints(pointVec); testNatural(sp, x, y); };
    { Ewoms::Spline<double> sp; sp.setContainerOfTuples(pointsInitList); testNatural(sp, x, y); };
}
...
void plot()
{
    ...
    Ewoms::Spline<double> spFull(xs, ys, m1, m2);
    Ewoms::Spline<double> spNatural(xs, ys);
    Ewoms::Spline<double> spPeriodic(xs, ys, /*type=*/Ewoms::Spline<double>::Periodic);
    Ewoms::Spline<double> spMonotonic(xs, ys, /*type=*/Ewoms::Spline<double>::Monotonic);
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        std::cout << "Caught eWoms exception: " << e.what() << "\n";
    }
