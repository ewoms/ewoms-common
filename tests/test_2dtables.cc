...
struct Test
{
    ...
    template <class Fn>
    std::shared_ptr<Ewoms::UniformTabulated2DFunction<Scalar> >
    createUniformTabulatedFunction(Fn& f)
    {
        ...
        auto tab = std::make_shared<Ewoms::UniformTabulated2DFunction<Scalar>>(
            xMin, xMax, m,
            yMin, yMax, n);
    ...
    template <class Fn>
    std::shared_ptr<Ewoms::UniformXTabulated2DFunction<Scalar> >
    createUniformXTabulatedFunction(Fn& f)
    {
        ...
        Scalar yMax = 1/3.0;
        unsigned n = 40;
        auto tab = std::make_shared<Ewoms::UniformXTabulated2DFunction<Scalar>>(Ewoms::UniformXTabulated2DFunction<Scalar>::InterpolationPolicy::Vertical);
        for (unsigned i = 0; i < m; ++i) {
            Scalar x = xMin + Scalar(i)/(m - 1) * (xMax - xMin);
            tab->appendXPos(x);
            for (unsigned j = 0; j < n; ++j) {
    ...
    template <class Fn>
    std::shared_ptr<Ewoms::IntervalTabulated2DFunction<Scalar> >
    createIntervalTabulated2DFunction(Fn& f)
    {
        ...
        return std::make_shared<Ewoms::IntervalTabulated2DFunction<Scalar>>(xSamples, ySamples, data, true, true);
    }
