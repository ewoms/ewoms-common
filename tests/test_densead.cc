...
struct TestEnvBase
{
    ...
    {
        // test the constructors of the Ewoms::DenseAd::Evaluation class
        const Scalar c = 1.234;
        const Scalar x = 4.567;
        const Scalar y = 8.910;
        Scalar z EWOMS_UNUSED = Ewoms::blank(tolerance);
        const Eval cEval = asImp_().createConstant(c);
        const Eval c2Eval EWOMS_UNUSED = asImp_().createConstant(c);
        const Eval xEval = asImp_().createVariable(x, 0);
        const Eval yEval = asImp_().createVariable(y, 1);
        const Eval zEval EWOMS_UNUSED = Ewoms::blank(xEval);
    ...
    {
        typedef Ewoms::MathToolbox<Eval> EvalToolbox;
...
// prototypes
    ...
    inline void testAll()
    {
        ...
        test2DFunction1(Ewoms::DenseAd::min<Scalar, numVars, staticSize>,
                        myScalarMin,
                        -1000, 1000,
                        /*p=*/1.234);

        test2DFunction2(Ewoms::DenseAd::min<Scalar, numVars, staticSize>,
                        myScalarMin,
                        /*T=*/1.234,
                        -1000, 1000);

        std::cout << "  Testing max()\n";
        test2DFunction1(Ewoms::DenseAd::max<Scalar, numVars, staticSize>,
                        myScalarMax,
                        -1000, 1000,
                        /*p=*/1.234);

        test2DFunction2(Ewoms::DenseAd::max<Scalar, numVars, staticSize>,
                        myScalarMax,
                        /*T=*/1.234,
                        -1000, 1000);
        ...
        test1DFunction(Ewoms::DenseAd::abs<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::abs));

        std::cout << "  Testing sqrt()\n";
        test1DFunction(Ewoms::DenseAd::sqrt<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::sqrt));

        std::cout << "  Testing sin()\n";
        test1DFunction(Ewoms::DenseAd::sin<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::sin),
                       0, 2*M_PI);

        std::cout << "  Testing asin()\n";
        test1DFunction(Ewoms::DenseAd::asin<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::asin),
                       -1.0, 1.0);

        std::cout << "  Testing cos()\n";
        test1DFunction(Ewoms::DenseAd::cos<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::cos),
                       0, 2*M_PI);

        std::cout << "  Testing acos()\n";
        test1DFunction(Ewoms::DenseAd::acos<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::acos),
                       -1.0, 1.0);

        std::cout << "  Testing tan()\n";
        test1DFunction(Ewoms::DenseAd::tan<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::tan),
                       -M_PI / 2 * 0.95, M_PI / 2 * 0.95);

        std::cout << "  Testing atan()\n";
        test1DFunction(Ewoms::DenseAd::atan<Scalar, numVars, staticSize>,
        ...
        test1DFunction(Ewoms::DenseAd::exp<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::exp),
                       -100, 100);

        std::cout << "  Testing log()\n";
        test1DFunction(Ewoms::DenseAd::log<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::log),
                       1e-6, 1e9);

        std::cout << "  Testing log10()\n";
        test1DFunction(Ewoms::DenseAd::log10<Scalar, numVars, staticSize>,
                       static_cast<Scalar (*)(Scalar)>(std::log10),
                       1e-6, 1e9);

        while (false) {
            Scalar val1 EWOMS_UNUSED = 0.0;
            Scalar val2 EWOMS_UNUSED = 1.0;
            Scalar resultVal EWOMS_UNUSED;
            Eval eval1 EWOMS_UNUSED = asImp_().createConstant(1.0);
            Eval eval2 EWOMS_UNUSED = asImp_().createConstant(2.0);
            Eval resultEval EWOMS_UNUSED = asImp_().newBlankEval();;

            // make sure that the convenince functions work (i.e., that everything can be
            // accessed without the MathToolbox<Scalar> detour.)
            resultVal = Ewoms::constant<Scalar>(val1);
            resultVal = Ewoms::variable<Scalar>(val1, /*idx=*/0);
            resultVal = Ewoms::decay<Scalar>(val1);
            resultVal = Ewoms::scalarValue(val1);
            resultVal = Ewoms::getValue(val1);
            resultVal = Ewoms::min(val1, val2);
            resultVal = Ewoms::max(val1, val2);
            resultVal = Ewoms::atan2(val1, val2);
            resultVal = Ewoms::pow(val1, val2);
            resultVal = Ewoms::abs(val1);
            resultVal = Ewoms::atan(val1);
            resultVal = Ewoms::sin(val1);
            resultVal = Ewoms::asin(val1);
            resultVal = Ewoms::cos(val1);
            resultVal = Ewoms::acos(val1);
            resultVal = Ewoms::sqrt(val1);
            resultVal = Ewoms::exp(val1);
            resultVal = Ewoms::log(val1);

            resultEval = Ewoms::decay<Eval>(eval1);
            resultVal = Ewoms::decay<Scalar>(eval1);
            resultVal = Ewoms::scalarValue(eval1);
            resultVal = Ewoms::getValue(eval1);
            resultEval = Ewoms::min(eval1, eval2);
            resultEval = Ewoms::min(eval1, val2);
            resultEval = Ewoms::max(eval1, eval2);
            resultEval = Ewoms::max(eval1, val2);
            resultEval = Ewoms::atan2(eval1, eval2);
            resultEval = Ewoms::atan2(eval1, val2);
            resultEval = Ewoms::pow(eval1, eval2);
            resultEval = Ewoms::pow(eval1, val2);
            resultEval = Ewoms::abs(eval1);
            resultEval = Ewoms::atan(eval1);
            resultEval = Ewoms::sin(eval1);
            resultEval = Ewoms::asin(eval1);
            resultEval = Ewoms::cos(eval1);
            resultEval = Ewoms::acos(eval1);
            resultEval = Ewoms::sqrt(eval1);
            resultEval = Ewoms::exp(eval1);
            resultEval = Ewoms::log(eval1);
            resultEval = Ewoms::log10(eval1);
...
struct DynamicTestEnv : public TestEnvBase<Ewoms::DenseAd::DynamicEvaluation<Scalar, staticSize>,
                                           -1,
                                           staticSize,
                                           Scalar,
                                           DynamicTestEnv<Scalar, staticSize> >
{
    typedef Ewoms::DenseAd::DynamicEvaluation<Scalar, staticSize> Eval;
    ...
    { return Ewoms::constant<Scalar, staticSize>(numDerivs_, c); }

    Eval createVariable(Scalar v, int varIdx) const
    { return Ewoms::variable<Scalar, staticSize>(numDerivs_, v, varIdx); }
...
struct StaticTestEnv : public TestEnvBase<Ewoms::DenseAd::Evaluation<Scalar, numDerivs>,
                                          numDerivs,
                                          staticSize,
                                          Scalar,
                                          StaticTestEnv<Scalar, numDerivs> >
{
    typedef Ewoms::DenseAd::Evaluation<Scalar, numDerivs> Eval;
    ...
    Eval createConstant(Scalar c) const
    { return Ewoms::constant<Eval, Scalar>(c); }

    Eval createVariable(Scalar v, int varIdx) const
    { return Ewoms::variable<Eval, Scalar>(v, varIdx); }
