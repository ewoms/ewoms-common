...
/*!
 * \file
 * \copydoc Ewoms::Tabulated1DFunction
 */
#ifndef EWOMS_TABULATED_1D_FUNCTION_HH
#define EWOMS_TABULATED_1D_FUNCTION_HH
...
public:
     ...
     */
    template <class Evaluation>
    Evaluation evalSecondDerivative(const Evaluation& x EWOMS_UNUSED, bool extrapolate EWOMS_UNUSED = false) const
    { return 0.0; }
     ...
     */
    template <class Evaluation>
    Evaluation evalThirdDerivative(const Evaluation& x EWOMS_UNUSED, bool extrapolate EWOMS_UNUSED = false) const
    { return 0.0; }
...
private:
    ...
    size_t findSegmentIndex_(const Evaluation& x, bool extrapolate = false) const
    {
        if (!extrapolate && !applies(x))
            throw Ewoms::NumericalIssue("Tried to evaluate a tabulated function outside of its range");
