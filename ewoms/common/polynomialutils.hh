 ...
 */
#ifndef EWOMS_POLYNOMIAL_UTILS_HH
#define EWOMS_POLYNOMIAL_UTILS_HH
...
unsigned invertLinearPolynomial(SolContainer& sol,
                                ...
                                Scalar b)
{
    if (std::abs(Ewoms::scalarValue(a)) < 1e-30)
        return 0;
                                       ...
                                       Scalar d)
{
    ...
    for (int i = 0; i < numSol; ++i) {
        ...
        if (std::abs(Ewoms::scalarValue(fPrime)) < 1e-30)
            continue;
        x -= fOld/fPrime;

        Scalar fNew = d + x*(c + x*(b + x*a));
        if (std::abs(Ewoms::scalarValue(fNew)) < std::abs(Ewoms::scalarValue(fOld)))
            sol[i] = x;
    }
}
//! \endcond
                               ...
                               Scalar d)
{
    ...
    if (std::abs(Ewoms::scalarValue(p)) > 1e-30 && std::abs(Ewoms::scalarValue(q)) > 1e-30) {
        // At this point
        //
        // t^3 + p*t + q = 0
        //
        ...
        if (wDisc >= 0) { // the positive discriminant case:
            ...
            Scalar u = - q/2 + Ewoms::sqrt(wDisc);
            if (u < 0) u = - Ewoms::pow(-u, 1.0/3);
            else u = Ewoms::pow(u, 1.0/3);
            ...
            Scalar uCubedRe = - q/2;
            Scalar uCubedIm = Ewoms::sqrt(-wDisc);
            // calculate the cube root of - q/2 + sqrt(q^2/4 + p^3/27)
            Scalar uAbs  = Ewoms::pow(Ewoms::sqrt(uCubedRe*uCubedRe + uCubedIm*uCubedIm), 1.0/3);
            Scalar phi = Ewoms::atan2(uCubedIm, uCubedRe)/3;
            ...
            for (int i = 0; i < 3; ++i) {
                sol[i] = Ewoms::cos(phi)*(uAbs - p/(3*uAbs)) - b/3;
                phi += 2*M_PI/3;
            }
    ...
    // divisions by zero in the code above...
    else if (std::abs(Ewoms::scalarValue(p)) < 1e-30 && std::abs(Ewoms::scalarValue(q)) < 1e-30) {
        ...
        Scalar t;
        if (-q > 0) t = Ewoms::pow(-q, 1./3);
        else t = - Ewoms::pow(q, 1./3);
        sol[0] = t - b/3;

        return 1;
    }

    assert(std::abs(Ewoms::scalarValue(p)) > 1e-30 && std::abs(Ewoms::scalarValue(q)) > 1e-30);
    ...
    sol[0] = -Ewoms::sqrt(-p) - b/3;;
    sol[1] = 0.0 - b/3;
    sol[2] = Ewoms::sqrt(-p) - b/3;
