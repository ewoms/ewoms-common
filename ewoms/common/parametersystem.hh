 ...
 *
 * In eWoms, parameters can only be used after they have been
 * registered.
 *
 * Example:
 *
...
/// \cond 0
inline int noPositionalParameters_(std::set<std::string>& seenParams EWOMS_UNUSED,
                                   std::string& errorMsg,
...
inline std::string parseUnquotedValue_(std::string& s, const std::string& errorPrefix EWOMS_UNUSED)
{
    unsigned i;
    for (i = 0; i < s.size(); ++ i)
        if (std::isspace(s[i]))
    ...
    if (!helpPreamble.empty()) {
        for (int i = 1; i < argc; ++i) {
            ...
            if (std::string("--help-all") == argv[i]) {
                printUsage<TypeTag>(helpPreamble, /*errorMsg=*/"", std::cout, /*printAll=*/true);
                return "Help called";
            }
        }
    }
...
public:
    ...
    template <class ParamType, class PropTag>
    static bool isSet(const char *propTagName EWOMS_OPTIM_UNUSED,
                      const char *paramName EWOMS_OPTIM_UNUSED,
                      bool errorIfNotRegistered = true)
    {
...
private:
    ...
    template <class ParamType, class PropTag>
    static const ParamType retrieve_(const char EWOMS_OPTIM_UNUSED *propTagName,
                                     const char *paramName,
                                     bool errorIfNotRegistered = true)
    {
#ifndef NDEBUG
...
void hideParam(const char *paramName)
{
    ...
    static const auto defaultValue EWOMS_UNUSED = GET_PROP_VALUE_(TypeTag, PropTag);
