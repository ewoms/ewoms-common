 ...
 * \file
 *
 * \copydoc Ewoms::UniformTabulated2DFunction
 */
#ifndef EWOMS_UNIFORM_TABULATED_2D_FUNCTION_HH
#define EWOMS_UNIFORM_TABULATED_2D_FUNCTION_HH
    ...
    Evaluation eval(const Evaluation& x, const Evaluation& y) const
    {
        ...
        if (!applies(x,y))
        {
            throw NumericalIssue("Attempt to get tabulated value for ("
                                   +std::to_string(double(Ewoms::scalarValue(x)))+", "+std::to_string(double(Ewoms::scalarValue(y)))
        ...
        unsigned i =
            static_cast<unsigned>(
                std::max(0, std::min(static_cast<int>(numX()) - 2,
                                     static_cast<int>(Ewoms::scalarValue(alpha)))));
        unsigned j =
            static_cast<unsigned>(
                std::max(0, std::min(static_cast<int>(numY()) - 2,
                                     static_cast<int>(Ewoms::scalarValue(beta)))));
