...
/*!
 * \file
 * \copydoc Ewoms::Spline
 */
#ifndef EWOMS_SPLINE_HH
#define EWOMS_SPLINE_HH

#include <ewoms/common/tridiagonalmatrix.hh>
...
class Spline
{
    typedef Ewoms::TridiagonalMatrix<Scalar> Matrix;
    ...
    */
    ...
    Evaluation eval(const Evaluation& x, bool extrapolate = false) const
    {
        if (!extrapolate && !applies(x))
            throw Ewoms::NumericalIssue("Tried to evaluate a spline outside of its range");
        ...
        return eval_(x, segmentIdx_(Ewoms::scalarValue(x)));
    }
    ...
    {
        if (!extrapolate && !applies(x))
            throw Ewoms::NumericalIssue("Tried to evaluate the derivative of a spline outside of its range");
        ...
        return evalDerivative_(x, segmentIdx_(Ewoms::scalarValue(x)));
    }
    ...
    {
        if (!extrapolate && !applies(x))
            throw Ewoms::NumericalIssue("Tried to evaluate the second derivative of a spline outside of its range");
        else if (extrapolate)
            return 0.0;

        return evalDerivative2_(x, segmentIdx_(Ewoms::scalarValue(x)));
    }
    ...
    {
        if (!extrapolate && !applies(x))
            throw Ewoms::NumericalIssue("Tried to evaluate the third derivative of a spline outside of its range");
        else if (extrapolate)
            return 0.0;

        return evalDerivative3_(x, segmentIdx_(Ewoms::scalarValue(x)));
    }

    /*!
     * \brief Find the intersections of the spline with a cubic
     *        polynomial in the whole interval, throws
     *        Ewoms::MathError exception if there is more or less than
     *        one solution.
     */
    template <class Evaluation>
     ...
     */
    int monotonic(Scalar x0, Scalar x1, bool extrapolate EWOMS_OPTIM_UNUSED = false) const
    {
...
protected:
     ...
     *
     * This code is inspired by ewoms-common's "MonotCubicInterpolator"
    ...
    template <class Evaluation>
    Scalar h00_prime3_(const Evaluation& t EWOMS_UNUSED) const
    { return 2*3*2; }

    template <class Evaluation>
    Scalar h10_prime3_(const Evaluation& t EWOMS_UNUSED) const
    { return 2*3; }

    template <class Evaluation>
    Scalar h01_prime3_(const Evaluation& t EWOMS_UNUSED) const
    { return -2*3*2; }

    template <class Evaluation>
    Scalar h11_prime3_(const Evaluation& t EWOMS_UNUSED) const
    { return 2*3; }
    ...
    {
        unsigned n =
            Ewoms::invertCubicPolynomial(sol,
                                       a_(segIdx) - a,
                                       b_(segIdx) - b,
                                       c_(segIdx) - c,
                                       d_(segIdx) - d);
