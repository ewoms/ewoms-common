...
#
# Usage: In the ewoms-common top-level source directory, run
 ...
 *        of variables in the localized automatic differentiation (AD) framework.
{% else %}\
 ...
 */
{% if numDerivs < 0 %}\
#ifndef EWOMS_DENSEAD_EVALUATION_DYNAMIC_HH
#define EWOMS_DENSEAD_EVALUATION_DYNAMIC_HH
{% elif numDerivs == 0 %}\
#ifndef EWOMS_DENSEAD_EVALUATION_HH
#define EWOMS_DENSEAD_EVALUATION_HH
{% else %}\
#ifndef EWOMS_DENSEAD_EVALUATION{{numDerivs}}_HH
...
{% if numDerivs < 0 %}\
#include <ewoms/common/fastsmallvector.hh>
{% else %}\
#include <array>
{% endif %}\
#include <cmath>
...
{% else %}\
    static Evaluation createBlank(const Evaluation& x EWOMS_UNUSED)
    { return Evaluation(); }
{% endif %}\
...
{% else %}\
    static Evaluation createConstantZero(const Evaluation& x EWOMS_UNUSED)
    { return Evaluation(0.); }
{% endif %}\
...
{% else %}\
    static Evaluation createConstantOne(const Evaluation& x EWOMS_UNUSED)
    { return Evaluation(1.); }
{% endif %}\
...
{% if numDerivs < 0 %}\
    template <class RhsValueType>
    static Evaluation createVariable(const RhsValueType& value EWOMS_UNUSED, int varPos EWOMS_UNUSED)
    {
...
{% else %}\
    ...
    template <class RhsValueType>
    static Evaluation createVariable(const Evaluation& x EWOMS_UNUSED, const RhsValueType& value, int varPos)
    {
        // copy function value and set all derivatives to 0, except for the variable
        // which is represented by the value (which is set to 1.0)
        return Evaluation(value, varPos);
...
{% if numDerivs < 0 %}\
    ...
    // relevant variables, f(x) = c).
    template <class RhsValueType>
    static Evaluation createConstant(const RhsValueType& value EWOMS_UNUSED)
    {
...
{% else %}\
    ...
    // relevant variables, f(x) = c).
    template <class RhsValueType>
    static Evaluation createConstant(const Evaluation& x EWOMS_UNUSED, const RhsValueType& value)
    {
        return Evaluation(value);
    }
{% endif %}\
...
Ewoms::DenseAd::Evaluation<Scalar, -1, staticSize> constant(int numDerivatives, const Scalar& value)
{ return Ewoms::DenseAd::Evaluation<Scalar, -1, staticSize>::createConstant(numDerivatives, value); }

template <class Scalar, unsigned staticSize>
Ewoms::DenseAd::Evaluation<Scalar, -1, staticSize> variable(int numDerivatives, const Scalar& value, unsigned idx)
{ return Ewoms::DenseAd::Evaluation<Scalar, -1, staticSize>::createVariable(numDerivatives, value, idx); }
...
namespace Dune {
template <class ValueType, int numVars, unsigned staticSize>
struct FieldTraits<Ewoms::DenseAd::Evaluation<ValueType, numVars, staticSize> >
{
public:
    typedef Ewoms::DenseAd::Evaluation<ValueType, numVars, staticSize> field_type;
...
{% elif numDerivs < 0 %}\
#endif // EWOMS_DENSEAD_EVALUATION_DYNAMIC_HH
{% else %}\
#endif // EWOMS_DENSEAD_EVALUATION{{numDerivs}}_HH
{% endif %}\
"""
...
fileName = "ewoms/common/densead/evaluation.hh"
...
fileName = "ewoms/common/densead/dynamicevaluation.hh"
...
for numDerivs in range(1, maxDerivs + 1):
    ...
    fileName = "ewoms/common/densead/evaluation%d.hh"%numDerivs
    specializationFileNames.append(fileName)
...
f = open("ewoms/common/densead/evaluationspecializations.hh", "w")
f.write(fileContents)
f.close()
